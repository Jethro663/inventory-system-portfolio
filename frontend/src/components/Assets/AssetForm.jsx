...
const AssetForm = ({ asset, onSubmit, onCancel }) => {
    const [formData, setFormData] = useState({
        name: '',
        serialNumber: '',
        categoryId: '',    // unified key: categoryId
        status: 'AVAILABLE',
        purchaseDate: '',
        cost: '',
        imageUrl: ''
    ...
    const handleImageUpload = async (e) => {
        ...
        if (!file) return;
        // same validations you had...
        const uploadData = new FormData();
        uploadData.append('file', file);
        const response = await api.post('/assets/upload', uploadData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        });
        // note: your api instance returns full axios response; URL inside response.data.data
